...
try {
            // 1 - instanciar o objeto do tipo Venda (SE FOR UMA NOVA VENDA)
            // Se this.venda não for nulo, significa que estamos editando
            if (this.venda == null) {
            ...
            venda = null; // Se der erro, anula o objeto
    ...
    private void cmbVeiculoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmbVeiculoActionPerformed
        ...
        Object selectedItem = cmbVeiculo.getSelectedItem();

        // Verifica se é uma instância de Veiculo (para evitar erro na inicialização)
        if (selectedItem instanceof Veiculo) {
            Veiculo veiculoSelecionado = (Veiculo) selectedItem;

            // Pega o valor do veículo
            double valor = veiculoSelecionado.getValor();

            // Atualiza o campo de texto
            // Usa String.format para garantir duas casas decimais e replace para usar vírgula
